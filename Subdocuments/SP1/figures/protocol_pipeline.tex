\begin{tikzpicture}[x=\spxunitstat,y=1.03cm]
\tikzset{
...
\node[factorBody] at (4.845,11.68)
  {$N,K$ · capacidad\\heterogeneidad · red\\perturbación · límite temporal};
...
\node[factorBody] at (11.145,11.66)
  {propuesta · referencias\\oráculo si certifica el óptimo};
...
\node[bandTitle] at (1.00,9.91) {Bucle Monte Carlo pareado};
...
\node[font=\fontsize{6.2}{6.9}\selectfont\sffamily\bfseries,
  text=SPGreen!45!black] at (12.225,8.54) {campaña completa};
...
% Resumen de la información conservada por ejecución.
...
\node[font=\fontsize{6.8}{7.5}\selectfont\sffamily\bfseries,
  text=VIUDark,anchor=west] at (0.95,7.04) {RESULTADO POR EJECUCIÓN};
\draw[draw=SPGrid!88,line width=0.45pt] (4.58,6.82) -- (4.58,7.26);
\node[font=\fontsize{6.2}{6.9}\selectfont\sffamily,text=VIUGray,
  anchor=west] at (4.78,7.04)
...
\node[font=\fontsize{6.2}{6.9}\selectfont\sffamily\bfseries,
  ...
  {FALLO/LÍMITE: visibles};
\draw[rule] (0.40,6.50) -- (13.08,6.50);

% C. Cálculo de métricas.
\node[tag] at (0.62,6.11) {C};
\node[bandTitle] at (1.00,6.11) {Cálculo de métricas y diferencias por mundo};
...
\node[boxTitle] at (1.55,5.45) {1 · DATOS};
...
\node[bandTitle] at (1.00,3.58) {Inferencia y contraste de la hipótesis};
...
\node[smallBody] at (0.62,2.41)
  {continua $\rightarrow$ signo exacto (Wilcoxon)};
...
\node[smallBody] at (5.35,1.83) {brecha: óptimo certificado};

\node[smallTitle] at (8.34,3.14) {DOS DECISIONES SEPARADAS};
\node[font=\fontsize{6.7}{7.4}\selectfont\sffamily,
  text=VIUGray,anchor=west] at (8.34,2.82)
  {Fijar $H_0$, $H_1$, $\alpha$ y, si procede, el umbral práctico};
\fill[SPBlue!3,rounded corners=2pt] (8.34,2.17) rectangle (12.90,2.70);
\draw[draw=SPBlue!45,rounded corners=2pt] (8.34,2.17) rectangle (12.90,2.70);
\node[font=\fontsize{6.25}{6.8}\selectfont\sffamily\bfseries,
  text=VIUDark,align=center] at (10.62,2.435)
  {INFERENCIA · $p_{\mathrm{Holm}}\leq\alpha$\\rechazar / no rechazar $H_0$};
\fill[VIUOrange!3,rounded corners=2pt] (8.34,1.51) rectangle (12.90,2.04);
\draw[draw=VIUOrange!52,rounded corners=2pt] (8.34,1.51) rectangle (12.90,2.04);
\node[font=\fontsize{6.2}{6.75}\selectfont\sffamily\bfseries,
  text=VIUDark,align=center] at (10.62,1.775)
  {RELEVANCIA · efecto e IC frente al margen\\cumple / no cumple el umbral};
\node[font=\fontsize{5.9}{6.5}\selectfont\sffamily,text=VIUGray,
  align=center] at (10.62,1.18)
  {respaldo operativo conjunto solo cuando\\se preespecificaron ambas condiciones};
