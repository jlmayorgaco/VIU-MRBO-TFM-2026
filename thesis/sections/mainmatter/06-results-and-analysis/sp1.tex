...
SP1 sustituye la demanda unitaria por cuotas $n_k\in\{1, 2, 3, 4\}$. Con demanda realizable, déficit y exceso caracterizan el cierre; bajo escasez, el déficit lineal no distingue concentrar robots en una carga de dispersarlos. El beneficio de cuórum rompe esa indiferencia y el operador QR produce grupos enteros. Las poses permanecen fijas y solo determinan el coste de reclutamiento. El movimiento se incorpora a partir de SP4.

\subsubsection{Cuotas y escasez}

La razón $\rho_D=(\sum_k n_k)/N$ distingue abundancia, ocupación crítica y escasez; las flechas de la Figura~\ref{fig:sp1-scenario} indican pertenencia lógica.
...
\begin{figure}[!htbp]
  ...
  \end{spdiagram}
  \caption{Regímenes lógicos de SP1. En abundancia se cierran todas las cuotas y quedan robots libres; en el régimen crítico se usa exactamente la flota; bajo escasez debe seleccionarse qué cargas completar.}
...
\end{equation}
Una carga cerrada satisface $q_k=|C_k|=n_k$. La cardinalidad no expresa masa, fuerza, torque ni compatibilidad; esas dimensiones aparecen en SP2 y el certificado mecánico en SP3.
...
\end{aligned}
\end{equation}
La igualdad de activación excluye grupos parciales y sobreasignados. El parámetro $\alpha=0.15$ pondera coste y valor normalizados. El modelo es un oráculo estático central.

La cardinalidad variable no vuelve NP-difícil todo SP1. Si todas las cargas son obligatorias y $\rho_D\leq1$, se fija $y_k=1$ y cada cuota se expande en $n_k$ plazas: resulta una asignación bipartita con cuotas o un flujo de coste mínimo polinómico. Si las cargas son opcionales, incluso con costes nulos, queda $\sum_kn_ky_k\leq N$ y $\max\sum_kv_ky_k$, que contiene mochila 0--1. La prueba se conserva en el Anexo~\ref{app:sp1-complexity}. La dificultad combinatoria aparece al seleccionar grupos completos bajo escasez, no por la cuota aislada.

\subsubsection{Métodos de comparación}

MILP-Q fija el mejor valor alcanzable con información global; Voraz-Q mide la calidad de una selección secuencial simple. Smith genera preferencias continuas y QR las transforma en grupos de tamaño exacto, por lo que la comparación separa el efecto del cuórum del efecto del cierre. El experimento usa ocupación global en ambos pasos. En la Tabla~\ref{tab:sp1-methods}, $T_S$ denota los pasos Smith e $I_Q\leq K$ los cierres de carga.
...
\begin{table}[!htbp]
 ...
 \label{tab:sp1-methods}
  ...
  Voraz-Q (familia de recursos \parencite{zhangParker2013Resources}) & Selección opcional NP-difícil; heurística & $O(K^2N\log N)$ & Central; robots libres y todos los costes & $\alpha$, orden & Implementación propia; grupos enteros, sin cota y dependiente del orden. \\
  ...
  Smith-QR & Cierre entero del problema NP-difícil & Smith $+\ {\scriptscriptstyle O(I_QKN\log N)}$;\allowbreak peor $O(K^2N\log N)$ & Preferencias, ordenación global y cuotas & Los anteriores, $\alpha$ & Ocupaciones $0$ o $n_k$; no óptimo ni plenamente distribuido. \\
...
CBBA estándar es mono-ganador y no es equivalente sin una extensión verificada \parencite{choiBrunetHow2009CBBA}; por ello no se ejecuta. \Textcite{shehoryKraus1998Coalition} y \textcite{vigAdams2006Coalition} aportan comparación conceptual, no filas experimentales.
...
\end{proposicion}
\end{contribucion}
Si existe déficit y un robot libre, incorporarlo mejora el potencial; si no hay robot libre, la realizabilidad obliga a que exista exceso y un traslado reduce ambas penalizaciones. Retirar exceso también mejora. En sentido inverso, toda desviación desde una asignación exacta crea déficit, exceso o ambos por un coste superior al posible ahorro espacial. La demostración completa está en el Anexo~\ref{app:sp1-feasible-proof}.
...
El caso mínimo contiene dos cargas de cuota dos y dos robots (Figura~\ref{fig:sp1-scarcity-counterexample}). Ambos perfiles tienen déficit total dos, pero solo uno completa una carga.
\begin{figure}[!htbp]
 \centering
 \begin{spdiagram}
...
La relajación mantiene preferencias $\rho_{ik}\in[0, 1]$ sobre inactividad y cargas. Con $\widehat q_k=\sum_i\rho_{ik}$, la fitness implementada usa el incremento marginal de \eqref{eq:sp1-quorum-benefit}, el coste espacial y la penalización de exceso. La dinámica Smith intercambia masa entre acciones con mejor fitness; su ejecución digital aplica Euler proyectado con $\Delta t=0.15$, hasta 250 pasos y tolerancia $10^{-6}$. La acción preferida $a_i^{\mathrm{cont}}=\argmax_k\rho_{ik}$ no se interpreta como coalición ejecutable.
...
\end{equation}
QR agrega de forma determinista la matriz completa para aislar el efecto del cierre. Los resultados verifican \eqref{eq:sp1-qr-invariants}; la realización vecinal y su coste de consenso permanecen pendientes.
\end{contribucion}
...
\end{equation}
En demanda realizable y con cargas obligatorias se fija $y_k=1$, de modo que $z_{1,k}=0$ significa cuota exacta. Bajo escasez no es alcanzable imponer $q_k=n_k$ para todas las cargas; la referencia debe incluir la selección todo-o-nada: $y_k=1$ para cargas cerradas y $y_k=0$ para las abandonadas. Así, el objetivo de regulación ejecutable es $q_k\in\{0,n_k\}$, no llenar simultáneamente todas las cuotas.
...
\end{corolario}
El corolario se deriva de la propiedad de mejora finita y de la caracterización de Nash demostradas en el Anexo~\ref{app:sp1-feasible-proof}. No se transfiere automáticamente a Euler proyectado, a Smith-QR ni al caso escaso: para esos mecanismos no se ha demostrado estabilidad del lazo muestreado ni optimalidad de la selección $y$. Las posiciones siguen siendo datos estáticos usados en $\kappa_{ik}$; no hay todavía sensores de pose, entrada física $u_i$, saturación de actuador, contacto o seguridad. SP2 cambia la salida regulada de conteo a capacidad, SP3 añade wrench y SP4 introduce el transporte entre poses.
...
Las 12 enumeraciones pequeñas reprodujeron la Proposición~\ref{thm:sp1-feasible-nash}; también se verificaron el contraejemplo y el beneficio del cuórum. Los 14 controles formales comparan el código con enunciados finitos, pero no prueban la convergencia del sistema Smith muestreado.
...
En demanda realizable, los Nash puros cumplen las cuotas bajo la cota demostrada. Bajo escasez, el cuórum rompe la indiferencia del déficit lineal y QR elimina grupos parciales, pero Voraz-Q conserva mayor valor. El resultado distingue preferencia continua de asignación ejecutable; SP2 sustituye la cuota por capacidad efectiva.
\endgroup
