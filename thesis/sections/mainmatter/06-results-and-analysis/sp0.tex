...
SP0 contiene \(N\geq K\) robots intercambiables y \(K\) cargas unitarias. Cada carga recibe exactamente un robot, cada robot atiende a lo sumo una carga y la posición interviene únicamente en el coste. La pregunta experimental es cuánto difiere un equilibrio factible del coste mínimo obtenido por asignación clásica.

Con \(\lambda>\kappa_{\max}\), todos los Nash puros son asignaciones factibles. El contraejemplo del Anexo~\ref{app:proof-sp0-poa} construye dos equilibrios cuya razón de costes crece sin cota, de modo que la eficiencia necesita un criterio adicional. El dominio de SP0 es la decisión lógica estática.
...
\begin{figure}[!htbp]
  ...
  \end{spdiagram}
  \caption{Exclusividad en SP0: las aristas grises son asociaciones admisibles, la azul es la selección final y \(R_5\) permanece libre cuando \(N>K\).}
...
\end{aligned}
\end{equation}
La integralidad de los vértices de la relajación reduce SP0 a asignación lineal. El algoritmo húngaro lo resuelve en \(O(N^3)\) tiempo y \(O(N^2)\) memoria \parencite{kuhn1955Hungarian}, por lo que SP0 pertenece a \(\mathsf P\). El Anexo~\ref{app:proof-sp0-complexity} demuestra la reducción y localiza la dificultad NP cuando se añade selección tipo mochila 0--1.
...
\begin{figure}[!htbp]
 \centering
 \includegraphics[width=0.98\textwidth]{../results/sp0/SP0_THEORY_v1/figures/fig-sp0-scaling-welfare.pdf}
 \caption{SP0 por tamaño de flota: tiempo de CPU observado, evaluaciones registradas y bienestar normalizado. Las operaciones no son homogéneas entre algoritmos y las curvas no sustituyen las cotas de complejidad.}
