\input{shared/generated-macros/sp2_numbers.tex}
...
El caso didáctico de la Figura~\ref{fig:sp2-scenario} fija $b_i^{\mathrm{res}}=0$, $\delta_{ik}=0$ y $(c_i^{\mathrm{pay}},b_i)\in\{(8,0.5),(8,0.5),(10,0.7),(10,0.5)\}$; por~\eqref{eq:sp2-discounts}--\eqref{eq:sp2-effective-capacity}, $e_{ik}=(4,4,7,5)$. Así, $C_k^A$ suma $8$ y $C_k^B$ suma $12$ frente a $d_k^{\mathrm{srv}}=10$.
...
\begin{figure}[!htbp]
  ...
  \begin{spdiagram}
    ...
    \node[spannotation,anchor=west] at (0.55, 3.45) {$e_{1k}=4$ unidades de servicio};
    \node[spannotation,anchor=west] at (0.55, 2.30) {$e_{2k}=4$ unidades de servicio};
    ...
    \node[spannotation,anchor=east] at (12.75, 3.45) {$e_{3k}=7$ unidades de servicio};
    \node[spannotation,anchor=east] at (12.75, 2.30) {$e_{4k}=5$ unidades de servicio};
  ...
  \end{spdiagram}
  \caption{Caso didáctico E2: dos coaliciones con cardinalidad dos y distinto servicio operacional; los valores no son observaciones experimentales.}
...
\end{equation}
En la realimentación de~\eqref{eq:sp2-feedback-candidate}, $F_i$ puede ser Smith, BNN, replicator o gradiente proyectado. La Proposición~\ref{prop:sp2-marginal-alignment} establece la identidad $\partial\Phi/\partial\rho_{ik}=f_{ik}^{\mathrm{marg}}$ para agregados exactos y $E$ fija. El cierre entero y la estabilidad muestreada requieren resultados adicionales.
...
\begin{table}[!htbp]
  ...
  \resizebox{\textwidth}{!}{\input{shared/generated-macros/sp2_comparison.tex}}
...
\begin{table}[!htbp]
  ...
  \resizebox{0.82\textwidth}{!}{\input{shared/generated-macros/sp2_ablation.tex}}
  \viusource{elaboración propia a partir de los datos procesados de E2}
\end{table}
