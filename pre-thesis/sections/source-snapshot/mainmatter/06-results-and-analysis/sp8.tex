\input{shared/generated-macros/sp8_numbers.tex}

\subsubsection{Etapa 3.2: escala y coordinación bajo red imperfecta}
\label{subsubsec:sp3-network}

Cuando dos componentes de la red dejan de observar una interacción común, cada uno puede alcanzar un Nash visible y conservar un conflicto global. E8 mide este fallo al escalar las coaliciones discretas de E7 y variar radio, retardo y pérdida. El número de robots es $N=2A$ y la campaña no incorpora una planta física.

\paragraph{Escala y observabilidad}
...
\begin{figure}[!htbp]
  \centering
  \begin{spdiagram}
    \spdrawarena{E8 \textbar\ escala, vecindad y pérdida de información}
  ...
  \end{spdiagram}
  \caption{Incremento de E8: el juego regula conflictos visibles, mientras el retardo, la pérdida y la partición alteran la información disponible.}
...
\begin{table}[!htbp]
  \centering
  \caption{Métodos y literatura comparada para E8.}
...
\paragraph{Juego sobre información visible}
...
\estadoaporte{demostrado bajo coherencia de copias y para la clase indistinguible declarada.}
La función
\[
 \Phi_8^G(\boldsymbol r)=-\sum_i b_{i r_i}^8-\lambda_8P_8^G(\boldsymbol r)
\]
es un potencial exacto de~\eqref{eq:sp8-visible-utility}. Todo camino maximal asíncrono de mejoras estrictas termina en un Nash visible tras, como máximo, $\prod_i|\mathcal R_i^8|-1$ cambios aceptados.
...
\end{proposicion}
Una partición no siempre colisiona: prioridades o mapa previo resuelven familias particulares. La imposibilidad exige el par de instancias localmente indistinguibles y decisiones óptimas incompatibles del enunciado (Anexo~\ref{app:sp8-proofs}).
\end{contribucion}
...
Las rutas abstraen los cuerpos de E3--E4. Por ello, los conflictos y el Nash pertenecen a la capa estratégica y no describen contacto, seguimiento o barreras físicas.
...
\begin{table}[!htbp]
  \centering
  \caption{Diseño experimental de E8.}
  ...
  \begin{tabularx}{0.94\textwidth}{>{\raggedright\arraybackslash}p{2.4cm} Y}
    ...
    \bottomrule
  \end{tabularx}
  \viusource{elaboración propia a partir del protocolo experimental de E8}
\end{table}
...
\begin{table}[!htbp]
  \centering
  \caption{Calidad y coste medios de los métodos ejecutados en E8.}
  \label{tab:sp8-results}
  \resizebox{0.94\textwidth}{!}{\input{shared/generated-macros/sp8_results.tex}}
  \viuownsource
\end{table}
