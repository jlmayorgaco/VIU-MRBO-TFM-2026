\subsection{Demostraciones complementarias de la etapa E6 (SP2)}
\label{app:sp6-proofs}

E6-C estudia una carga afectada por un fallo simple mediante recursos aditivos no negativos, costes positivos y una reserva finita. Esta estructura permite caracterizar el juego binario y su tiempo de reparación. Los contactos friccionales, la competencia entre cargas y los fallos simultáneos requieren un modelo acoplado distinto; la planta posterior al reacoplamiento conserva su propio certificado.
...
La finitud y no vacuidad de \(\{0,1\}^{n_R}\) garantizan que \(\Phi_6\) alcanza un máximo global, donde ninguna desviación unilateral mejora la utilidad. Existe, por ello, al menos un Nash puro. En una trayectoria de mejoras estrictas, el aumento de \(\Phi_6\) impide repetir perfiles. Sus \(2^{n_R}\) elementos acotan la trayectoria por \(2^{n_R}-1\) cambios aceptados, y el perfil terminal es Nash.

\ifdefined\VIUFullSPsixProofs
...
\end{equation}
Como cada robot dispone de dos acciones, las desigualdades~\eqref{eq:sp6-ne-entry}--\eqref{eq:sp6-ne-exit} son necesarias y suficientes para caracterizar los Nash puros.
\fi
...
Para probar minimalidad, sea $\boldsymbol x$ un Nash factible y supóngase que un miembro $i$ puede salir conservando factibilidad. Entonces
$D_6(\boldsymbol x-\boldsymbol e_i)=D_6(\boldsymbol x)=0$ y
\[
 \Phi_6(\boldsymbol x-\boldsymbol e_i)-\Phi_6(\boldsymbol x)=\kappa_i^6>0,
\]
lo que contradice la condición de Nash. Cada miembro es crítico para al menos una componente.
...
La positividad de los costes hace mínima por inclusión a toda solución factible de coste mínimo. Por la igualdad anterior pertenece al conjunto de Nash, de modo que $\operatorname{PoS}_6=1$.
\ifdefined\VIUFullSPsixProofs
En el contraejemplo de la Sección~\ref{app:sp6-poa-proof}, el cociente del peor Nash diverge y establece $\operatorname{PoA}_6=+\infty$.
\else
El contraejemplo paramétrico conservado en la monografía técnica muestra que el cociente del peor Nash diverge; la memoria no repite esa construcción.
\fi
...
$\max_{i:x_i=1}\ell_i/v_i^{\min}$. Añadir el tiempo de asentamiento $\tau_s$ produce~\eqref{eq:sp6-time-bound}. Si esa suma no supera $T_{\mathrm{safe}}$, la restauración ocurre antes del plazo bajo los supuestos de trayectoria libre y velocidad garantizada. La cota caracteriza el tiempo estratégico y de aproximación; la seguridad ante obstáculos y la estabilidad posterior al contacto se certifican en sus capas respectivas.
...
El perfil $\boldsymbol x^A=(1, 0, 0)$ es factible y cuesta $M$. El robot 1 no abandona porque hacerlo cambia el coste penalizado de $M$ a $\lambda_6>M$. Los robots 2 y 3 no entran porque serían redundantes y añadirían coste. Las dos desigualdades de desviación hacen a \(\boldsymbol x^A\) un perfil Nash.
...
El cociente entre el peor Nash y el óptimo es \(M/2\). Su divergencia cuando \(M\to\infty\) descarta una cota uniforme de eficiencia para el peor Nash, incluso bajo el umbral de factibilidad.

\subsection{Comprobación automática}

Para cada mundo confirmatorio, la auditoría enumera todos los perfiles binarios. Un bloque verifica la identidad de potencial y las condiciones de Nash; otro comprueba factibilidad y minimalidad, y un tercero registra el ascenso y la cota de \(2^{n_R}-1\) cambios. Esta comprobación finita complementa las pruebas analíticas y hereda los supuestos del modelo aditivo.
\fi
