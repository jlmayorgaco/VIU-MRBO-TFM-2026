...
% formales y el demostrador extremo a extremo. Fuente: revisión matemática
% interna (nota~\ref{fn:megajuego-source}). Las demostraciones completas de
...
completo.\footnote{\label{fn:megajuego-source}Fuente:
\parencite{mrobMegajuegoRevision2026}, revisión matemática interna, 16 de
septiembre de 2026. Corrige una formulación previa de la misma serie técnica
que asumía convergencia al óptimo global; aquí cada bloque certifica solo una
mejora local bajo hipótesis explícitas.} Su estado aumentado
\begin{equation}
...
\paragraph{Capa epistemológica por diferencias (propuesta)}
La procedencia incluida en $\mathcal I$ puede ampliarse con una memoria local
que separe beliefs físicos, beliefs estratégicos y un digest de versiones. En
vez de reenviar el estado completo, cada AMR transmitiría al vecino únicamente
los hechos nuevos respecto de la última versión que ese vecino ha confirmado;
la edad, el número de saltos y la confianza modularían su uso en reclutamiento,
reservas y revisiones conjuntas. Esta es una formulación candidata, no una
capa ejecutada en el demostrador de esta sección: todavía no acredita menor
tráfico, un \emph{commit} más temprano ni robustez frente a pérdida o retardo.
El Anexo~\ref{app:megajuego-epistemic-detail} fija el estado, la regla de
admisión y sus límites, y el material suplementario conserva el protocolo A/B
necesario para evaluarla.
...
\begin{figure}[!htbp]
  ...
  \viusource{elaboración propia a partir de la revisión matemática interna
  citada en la nota~\ref{fn:megajuego-source}}
\end{figure}
...
\estadoaporte{demostrado, condicionado a testigo inicial seguro y ausencia de Zeno; demostración en el Anexo~\ref{app:megajuego-full-detail}.}
\begin{teorema}[Presupuesto de ejecución, condicionado]
\label{thm:megajuego-execution-budget}
Con testigo inicial seguro, tubos que componen, entradas locales realizables,
ausencia de Zeno y un presupuesto $W$ que decrece al menos $\kappa_W\Delta t$
durante el movimiento y en $\delta>0$ por cada sustitución admitida,
$\kappa_WT+\delta N_{\mathrm{rev}}\leq W_0$. La ejecución no puede prolongarse
más allá de $T\leq W_0/\kappa_W$ y admite a lo sumo
$\lfloor W_0/\delta\rfloor$ revisiones estrictas. La cota acota la duración y
el número de revisiones; no garantiza que se alcance el objetivo, que puede
quedar sin entregar al agotarse el presupuesto.
\end{teorema}
\end{contribucion}
...
\begin{figure}[!htbp]
  ...
  \viusource{simulación propia, \texttt{simulate\_e2e\_megagame.py}, escenario
  sintético descrito en la nota~\ref{fn:megajuego-source}}
\end{figure}
...
reclutamiento y del orden de coaliciones, que sí se comparan contra un
oráculo. El escenario, el simulador y las salidas completas (CSV, resumen
JSON, animación) se conservan en \texttt{pre-thesis/resources/MROB\_MegaGame\_E2E\_bundle/}.
