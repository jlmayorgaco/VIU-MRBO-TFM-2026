...
\input{sections/v2/floats/lit-tab-corpus-layers}
...
\begin{table}[!htbp]
  ...
  \begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}p{2.15cm} >{\raggedright\arraybackslash}p{3.05cm} >{\raggedright\arraybackslash}p{3.05cm} Y@{}}
    ...
    Qué documenta & mecanismos con formulación y garantía publicadas & capacidades que el fabricante declara en producto & desplazamiento temático del corpus de publicaciones patentarias recuperado \\
...
\input{sections/v2/floats/ind-fig-sequence}
...
\input{sections/v2/floats/lit-tab-prior-art}
...
\input{sections/v2/floats/ind-tab-standards}
...
lectura desde otro ángulo, con el alcance que permite un descriptor de grafo. Los
núcleos de coautoría fuerte son pequeños y aislados ---como máximo cuatro
autores---: el corpus recuperado no se organiza en torno a un grupo grande y
conectado. Las comunidades de método tienen modularidad baja
($Q=$\LitMethodModularity{}), lo que significa que, bajo esta codificación de
familias, este grafo de coocurrencia y este algoritmo de comunidades, consenso,
subasta, aprendizaje y metaheurística no se separan en bloques y aparecen
combinados en un mismo trabajo con frecuencia. Ninguna de las dos cifras
autoriza la afirmación más fuerte de que no exista una agenda o una escuela
dominante: eso exigiría un diseño de estudio distinto, sobre un corpus no
sesgado por la propia consulta de recuperación.
