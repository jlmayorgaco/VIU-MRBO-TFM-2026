...
def run_campaign(
    ...
    *,
    ...
    """Execute a declared campaign. Confirmatory execution needs explicit consent."""

    if config.mode == "confirmatory" and not authorize_confirmatory:
    ...
    try:
        backend.start()
        if config.mode == "confirmatory" and backend.metadata.backend_kind != "coppeliasim_mujoco":
            raise RuntimeError("confirmatory execution cannot use a non-Coppelia backend")
    ...
    physical_backend = backend.metadata.backend_kind == "coppeliasim_mujoco"
    ...
    calibration_gates = (
        "sensor_gate_pass", "force_transmission_gate_pass", "wheel_twist_gate_pass",
        "scene_actuation_audit_pass", "config_readback_gate_pass",
