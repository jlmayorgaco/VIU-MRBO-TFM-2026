...
| --- | ---: | ---: | ---: | ---: | ---: |
| Central MILP + preview | 0.00 | 10.45 | 10.45 | 79.19 | 0.00 |
| Auction + reciprocal proxy | 1.00 | 8.53 | 8.60 | 125.62 | 10.05 |
| Auction + predictive CBF proxy | 1.00 | 9.83 | 9.83 | 144.32 | 12.30 |
| Replicator + CBF (TFM) | 1.00 | 19.23 | 19.76 | 85.52 | 835.99 |

## Contrastes pareados de entregas

| A − B | Diferencia | IC95 % | p Holm | Decisión |
| --- | ---: | ---: | ---: | --- |
| Central MILP + preview − Auction + reciprocal proxy | -1.000 | [-1.000, -1.000] | 1.0000 | no concluyente |
| Central MILP + preview − Auction + predictive CBF proxy | -1.000 | [-1.000, -1.000] | 1.0000 | no concluyente |
| Central MILP + preview − Replicator + CBF (TFM) | -1.000 | [-1.000, -1.000] | 1.0000 | no concluyente |
...
La variante mantiene las estanterías pero retira el puesto y las barreras centrales. El método propuesto usa un campo de objetivo continuo, un juego Replicator local sobre primitivas izquierda/derecha/espera y proyección cíclica CBF; Euler lo ejecuta con el paso declarado. A* queda fuera de esta variante y se conserva como baseline independiente.
...
En las 8 ejecuciones se registraron **0** solapamientos estáticos, **0** solapamientos dinámicos y **0** fallos reales de A*. Las esperas y rutas temporalmente no disponibles se conservan aparte como reservas y aplazamientos de planificación.
La variante continua registró **0** intervenciones del guard de último recurso y un residual EXEC máximo de **6.531e-02**. RAW, SAFE y EXEC se conservan por separado; estos datos muestreados no constituyen una prueba de invariancia continua.
