...
def build_world_contract(
    ...
    """Build the exact SI contract hashed and acknowledged for one reset."""

    if reset_sequence <= 0:
        raise ValueError("reset_sequence must be positive")
    contract: dict[str, object] = {
        ...
        "active_robot_ids": [
            item.robot_id
            for item in config.backend.robots[: world.active_robot_count]
        ],
    ...
    contract["contract_hash"] = hashlib.sha256(serialized.encode("utf-8")).hexdigest()
    return contract


def validate_independent_scene_audit(scene: Path) -> dict[str, Any]:
    """Recompute every hash asserted by the independent scene audit."""
    ...
    passed = bool(
        ...
        and artifacts.get("scene", {}).get("sha256") == _sha256_file(scene)
    )
    return {
        "pass": passed,
...
class CoppeliaMuJoCoBackend:
    ...
    def _load_independent_scene_audit(self, scene: Path) -> dict[str, Any]:
        return validate_independent_scene_audit(scene)
    ...
    def _world_contract(self, world: WorldSpec, dt_s: float) -> dict[str, object]:
        ...
        # The sequence is deliberately excluded from WorldSpec. It proves
        # that an acknowledgement belongs to this reset while the physical
        # world remains identical across paired guard runs.
        return build_world_contract(
            self.config, world, dt_s, self._reset_sequence
        )
