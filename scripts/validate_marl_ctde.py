"""Evaluate the trained CTDE MARL baseline against warehouse comparators."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--model", type=Path, default=Path("results/marl_ctde_training/model.json"))
    parser.add_argument("--out", type=Path, default=Path("results/marl_ctde_validation"))
...
def main() -> int:
    ...
    for eval_case in selected_cases:
        ...
        for method in selected_methods:
            ...
            for seed in seeds:
                row = _run_one(eval_case, scenario, method, policy, params, seed, model, args)
                rows.append(row)
                print(
    ...
    deltas = paired_deltas(rows, "marl_ctde", tuple(method for method in selected_methods if method != "marl_ctde"))
    ...
    write_readme(args.out / "README.md", model, seeds, summary, deltas, tests)
    return 0
...
def _run_one(
    ...
    if method == "marl_ctde":
        overrides["marl_ctde_weights"] = tuple(float(value) for value in model["weights"])
        overrides["marl_ctde_idle_score"] = float(model["idle_score"])
...
def statistical_tests(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    output: list[dict[str, Any]] = []
    for baseline in ("marl_proxy", "smith_qr_full", "smith", "greedy"):
        diffs = paired_metric_values(rows, "marl_ctde", baseline, "reward_capture_ratio")
        if diffs.size < 2:
            continue
        ...
        output.append(
            {
                "hypothesis": f"marl_ctde_capture_gt_{baseline}",
                "h0": f"MARL-CTDE no mejora captura frente a {baseline}",
                "h1": f"MARL-CTDE mejora captura frente a {baseline}",
                "n": int(diffs.size),
                "alpha": 0.05,
