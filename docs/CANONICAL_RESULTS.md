...
The primary end-to-end numerical evidence is `PHYSICAL_COALITION_CERTIFICATE_v1_1_FIXEDN`: config `configs/experiments/physical_coalition/PHYSICAL_COALITION_CERTIFICATE_v1_1_FIXEDN.yaml`, hypotheses contract `configs/experiments/physical_coalition/PHYSICAL_COALITION_CERTIFICATE_v1_1_FIXEDN_hypotheses.yaml`, code under `src/viu_mrob_tfm/physical_coalition/`, and results under `results/physical_coalition/PHYSICAL_COALITION_CERTIFICATE_v1_1_FIXEDN/`. It executes the cumulative A0--FULL ladder on one Euler--Lagrange load model using a fixed design of 100 paired worlds per family: 400 independent worlds and 2,400 unique method runs, CPU only. Inference uses exact two-sided McNemar tests, paired percentile-bootstrap intervals with frozen seeds, and Holm correction across all 20 contrasts. Its claim scope is planar numerical simulation. FULL uses a global-candidate replacement proxy and retains a 0.11 collision rate in the obstacle/dropout family; it is not evidence of end-to-end local recovery or functional safety.

The adaptive `PHYSICAL_COALITION_CERTIFICATE_v1` directory is superseded and non-canonical. Its original sealed state remains in Git history; the current files include a post-freeze diagnostic rerun after a recovery correction and must not be combined with v1.1 fixed-$n$ estimates.
...
- SP4 docking game v3: 1,188 runs across 108 paired scenario instances grouped into 18 independent seed-by-fleet blocks, theory audit PASS, 0 initial collisions, 6 seeds, 11 methods, 6 scenarios and N={4,8,12}. The five frozen directions remain supported by exact block-level sign tests with Holm correction. Guarded methods had zero swept collisions; replicator safe success was 0.2685 with 0.7315 timeout.
