...
| SP1 | ¿Se forman coaliciones de tamaños variables? | Requisito de capacidad escalar distinto por carga y coaliciones de cardinalidad emergente | Juego finito de cuórum y benchmark final QPG con precios locales de cuota, compromiso atómico y recuperación aumentante común | MILP/LP de asignación con capacidad; Capacity-CBBA y Weighted-GRAPE/Pair como adaptaciones distribuidas explícitas; GRAPE-S fiel solo en el benchmark discreto E10 | B parcial por el juego finito y C extensa por `SP1_TFM_FINAL_QUOTA_GAME_BENCHMARK_v1`: 2.200 tareas E1--E10 más 10 controles E0, escala finita hasta \(N=500\), MILP para \(N\leq50\), comunicación reconstruible, censura, recovery y certificados auditados. `SP1_TFM_VALIDATION_CLOSURE_v1_1` añade atribución de semilla, 1.620 ejecuciones persistentes de nueve protocolos, Bernstein no vacuo, 2.506 certificados de gap y 180 filas dinámicas. El preview `SP1_SCALE_QPG_BENCHMARK_v2_preview` conserva atomicidad, versiones, potencial monótono, terminación strict-BR, mensajería y localidad, pero refuta la superioridad exigida: ambas variantes fallan los gates conjuntos de factibilidad, distancia, bytes e integridad; por ello V2 full no se ejecuta. No hay prueba de convergencia global, completitud ni superioridad general | Obligatorio |
| SP2 | ¿Se cubre un umbral operacional cuando la contribución depende del par robot--carga? | Índice adimensional de servicio: carga útil normalizada ponderada por disponibilidad de batería y distancia; compatibilidad universal en la campaña | Puntuación de déficit ponderada por contribución marginal; comparación separada de cobertura y completitud | MILP de puntuación y MILP de cobertura normalizada; voraz, aproximación CBBA y reglas de puntuación supervisadas | B parcial: proposición de alineación con $E$ fija + reanálisis extenso de observaciones archivadas; faltan generadores históricos completos, estimación vecinal y factibilidad mecánica | Obligatorio |
| SP3 | ¿La coalición es físicamente ejecutable bajo la modalidad seleccionada? | `SP3-C`: soporte, rigidez y wrench. `SP3-E`: contacto unilateral y confinamiento de empuje/caging | Cargo: potencial de formación rígida + reparto de wrench. Empuje/caging: selección de contactos, condición de caging cuando proceda y regulación local de empuje | Cargo: QP de fuerzas y estructura virtual. Empuje: planificador/control central de empuje y referencias de caging | B parcial para C mediante reanálisis histórico; el demostrador Cargo usa un criterio agregado distinto del QP; C para E pendiente | Cargo obligatorio; E secundaria |
| SP4 | ¿La carga llega a la pose objetivo bajo cada interfaz física? | `SP4-C`: docking dinámico y trayectoria del cuerpo compuesto. `SP4-E`: trayectorias de empuje cerradas con pose estimada de la caja | Cargo: juego de vivacidad distribuido + HOCBF para docking y servocontrol de pose mediante proyección acotada de wrench. Empuje/caging: extensión aún no ejecutada | Cargo: directo/CBF, planificador central+HOCBF, PD de pose y preview central. Empuje: planificación central de contactos/empujes y caging distribuido | B para garantías locales reanalizadas: docking y transporte se evaluaron en estratos separados; el demostrador híbrido compone uniciclos y carga reducida con mecanismos sustitutos, sin tracción rueda--suelo; E pendiente | Cargo obligatorio; E secundaria |
