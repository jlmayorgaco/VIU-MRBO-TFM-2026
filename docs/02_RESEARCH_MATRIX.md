...
| SP1 | ¿Se forman coaliciones de tamaños variables? | Requisito de capacidad escalar distinto por carga y coaliciones de cardinalidad emergente | Juego finito de cuórum y, como benchmark separado, DRD-simple con consenso dinámico y cierre entero común | MILP/LP de asignación con capacidad; CBBA-1-Capacity como adaptación distribuida explícita, no CBBA canónico | B parcial por el juego finito; evidencia C extensa para `SP1_DRD_VS_CBBA_SIMPLE_v1`: 920 mundos pareados, 3.680 filas primarias, escala hasta \(N=500\), comunicación reconstruible y MILP para \(N\leq50\). Falta prueba de convergencia de DRD, el cierre raw de DRD casi nunca es entero factible y recovery no acredita arquitectura distribuida completa | Obligatorio |
