...
|---|---|---|---|
| C1 | The project implements a reproducible SP1-SP8 suite for allocation, capacity, wrench feasibility, motion, obstacle transport and resilience, plus descriptive communication and exploratory mesoscopic scale studies. | `configs/experiments/sp1` through `sp8`, canonical manifests and tests | Defensible within the declared simulator of each SP |
...
| C5 | Canonical SP4 v3--SP6 provide simulated motion, post-docking payload transport and operational recovery evidence using quantitative tables plus qualitative videos. | SP4 v3, SP5 v2 and SP6 canonical results/tests | Defensible in the declared planar reduced-order simulators; not hardware or independent contact dynamics |
...
| C8 | SP8 explores quality trends for centralized, distributed and hierarchical alternatives in a method-aware mesoscopic generator from 5 to 50.000 AMR. Its declared timeouts and analytical memory do not measure observed intractability or RSS. | `results/sp8/SP8_MC_fleet_ladder_high_power/`; representative MP4 in `results/sp8/SP8_MC_scalability_warehouse/` | Exploratory only |
...
| L6 | SP8 proves industrial warehouse deployment. | "SP8 is an exploratory mesoscopic generator of quality trends at warehouse-scale nominal counts; resources require a common external watchdog." | "SP8 proves observed intractability, RSS scaling or a deployable industrial controller." |
| L8 | CoppeliaSim validates the dynamics. | "The retained CoppeliaSim artifact is a complementary kinematic replay of precomputed trajectories." | "CoppeliaSim independently validates closed-loop dynamics, contacts or hardware transfer." |
| L9 | The complete architecture is distributed. | "Several game updates use local or aggregate information; canonical thresholds, closures, guards and replacement still use global state." | "A0--FULL is distributed end to end." |
