...
\begin{equation}
  ...
  \label{eq:lookahead}
\end{equation}
El movimiento se obtiene minimizando localmente un campo de potencial definido
sobre el punto adelantado:
\begin{equation}
  F_i(\mathbf{h}_i) = \frac{\alpha}{2}\norm{\mathbf{h}_i - \mathbf{l}_{s_i}^{\mathrm{eff}}}^2
    + \eta \sum_{j \in \mathcal{N}_i}
      \frac{1}{\norm{\mathbf{h}_i - \mathbf{h}_j}^2 + \varepsilon_r^2}
...
segundo repele a los robots entre sí para evitar colisiones; y el tercero mantiene
la geometría de coalición. El parámetro $\varepsilon_r = 0{,}01\,\text{m}$
regulariza el denominador cuadrático evitando singularidades; la forma
...
Esta formulación regula prácticamente el punto adelantado $\mathbf{h}_i$ a una
vecindad del objetivo y evita aplicar directamente un gradiente cartesiano al centro
geométrico del robot diferencial, respetando así su naturaleza no holonómica.
...
transición de modo. Bajo las dinámicas de Smith
...
En consecuencia, el sistema implementado no coincide con el juego base del
Teorema~\ref{prop:convergencia}, sino con una versión perturbada según la
Proposición~\ref{prop:perturbacion}. Por tanto, no se afirma convergencia exacta al
equilibrio Nash ideal, sino \emph{convergencia práctica a una vecindad del
equilibrio}, cuyo radio se evalúa empíricamente mediante las métricas $F$, $D(R)$,
$\Delta_{\mathrm{chg}}$ y $\Delta J$.
...
el número de robots con estrategia $s_i = k$. En régimen estacionario, una vez que
los robots asignados alcanzan el radio de detección $r_{\mathrm{det}}$, se cumple
$|\mathcal{C}_k| = N\tilde{x}_k$. Entonces:
...
\end{lema}
\begin{proof}
Como $N\tilde{x}_k$ es entero: (i) $\tilde{x}_k \geq n_k/N$ implica
$N\tilde{x}_k \geq n_k$; (ii) $|N\tilde{x}_k - n| < 1/2$ con $N\tilde{x}_k$ y $n$
enteros fuerza $N\tilde{x}_k = n$.
\end{proof}
Este lema conecta el equilibrio continuo $x_k^* = n/N$ del
Teorema~\ref{prop:convergencia} con la factibilidad entera: basta una vecindad de
radio $1/(2N)$ alrededor de $x_k^*$ para garantizar la cardinalidad exacta.
...
$N_{\text{fail}}$ & Episodios fallidos &
  ...
  $|\mathcal{C}_k| \geq n_k$ dentro del horizonte; se reporta como tasa de fallo
  por tratamiento. \\
\bottomrule
\end{tabularx}
\end{table}
...
Conviene precisar el alcance de esta validación. La simulación en CoppeliaSim
verifica la \textbf{viabilidad cinemática} del esquema de asignación, formación y
navegación con robots Pioneer~3-DX (saturación de velocidades, geometría espacial).
\textbf{No constituye validación de la manipulación cooperativa por contacto} ni de
la distribución de fuerzas sobre la carga, que pertenece a una capa de control
complementaria \citep{ebel2024cooperative, rosenfelder2024force} fuera del alcance
de este TFM. La carga se representa como un cuboide pasivo acoplado cinemáticamente
al centroide de la coalición una vez formada.
