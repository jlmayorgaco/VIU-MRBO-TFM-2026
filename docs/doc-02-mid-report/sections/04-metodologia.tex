...
$n_k$. Responde a una \textbf{descomposición por capas}: la capa de
asignación (este trabajo) determina \emph{qué} robots forman la coalición y
...
disponible para cada robot. La notación que emplea el resto del capítulo queda fijada
en su formalización.
...
La tarea $k = 0$ representa el estado de inactividad, con recompensa base pequeña
$r_0 > 0$ (ec.~\eqref{eq:payoff_idle}) y $n_0 = 0$. El conjunto de estrategias disponibles para cada robot es
...
bajo cambios de estrategia individuales (un cambio individual desplaza las $K$
componentes del vector de estimación, de ahí la cota $O(K/N)$; $m$ reasignaciones
simultáneas escalan como $O(m/N)$, véase Obs.~\ref{obs:dac_simultaneo}) y recupera la
media exacta en tiempo $O(1/(\varepsilon\lambda_2))$ entre eventos \citep{kia2019tutorial}.
...
fuera del alcance de este trabajo (véase \S\,\ref{sec:conclusiones}, trabajo futuro). El
