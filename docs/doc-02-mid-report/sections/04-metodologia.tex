...
descentralización. La metodología se organiza en quince subsecciones que cubren
...
la tarea se evalúan empíricamente en los Escenarios~B, C y~E. Conviene subrayar,
asimismo, que las cotas \eqref{eq:betamin}--\eqref{eq:betamax} son condiciones
\emph{suficientes} derivadas del análisis de Lyapunov: valores de $\beta$ fuera del
intervalo pueden seguir siendo estables, pero el teorema no lo garantiza.
...
con igualdad sólo en los equilibrios de Nash \citep[\S5.6 y \S7.1]{sandholm2010population}.
% TODO-CITA: verificar contra el paper el localizador exacto (¿Sección/Teorema?) y que
% el resultado se enuncia para dinámicas de Smith discretas específicamente.
Para la versión en tiempo discreto empleada aquí, \citet{martinezpiazuelo2022tcss}
demuestran que las dinámicas de población distribuidas en tiempo discreto generan
...
el número de robots con estrategia $s_i = k$. En general
$|\mathcal{C}_k(t)| \leq N\tilde{x}_k(t)$, con igualdad cuando todos esos robots se
...
\begin{itemize}
  \item \textbf{T1 -- Heurística voraz local}: cada robot selecciona la tarea más
    ...
    revisión. Este tratamiento responde si basta con elegir la tarea cercana con
    déficit.
