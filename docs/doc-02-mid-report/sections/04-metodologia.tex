...
comparativo. El propósito es evaluar si las tres dinámicas acopladas propuestas
superan a cuatro tratamientos de referencia (T1 heurística voraz local, T2 consenso puro,
T3 replicador distribuido y T4 referencia centralizada de mínima distancia) bajo condiciones controladas
y reproducibles.
La metodología se organiza en catorce subsecciones que cubren el modelo del sistema,
el método propuesto, el análisis de convergencia, el diseño experimental y
...
Consídere $N$ robots con rango de comunicación $R = \infty$ (sin error de
consenso), $K$ tareas homógeneas con $n_k = n$ para todo $k$ y $r_k = r$ para
...
la condición de paso se corresponde con $\beta_{\max}$ (Paso~4). Bajo estas hipotésis,
...
\begin{itemize}
  \item \textbf{T1 -- Heurística voraz local}: cada robot selecciona la tarea más
    cercana que todavía necesita robots ($|\mathcal{C}_k| < n_k$). No hay consenso ni
    dinámicas de Smith.

  \item \textbf{T2 -- Consenso puro}: se ejecuta el consenso distribuido
    \eqref{eq:consenso}, pero la asignación usa la regla greedy sobre las
    estimaciones $\hat{x}_{ik}$ sin las dinámicas de Smith.

  \item \textbf{T3 -- Replicador distribuido (DRD)}: dinámica del replicador en
    ...
    mediante una segunda capa de consenso sobre la red (véase \S\,\ref{subsec:porquesmith}
    para la argumentación completa). Esta línea base permite cuantificar
    empíricamente el sesgo de estimación dependiente de topología que
    distingue Smith de DRD.

  \item \textbf{T4 -- Referencia centralizada de mínima distancia}: con
    acceso al estado global, se resuelve la asignación de mínimo coste
    respetando la cardinalidad: cada tarea $k$ se expande en $n_k$ \emph{slots}
    idénticos; se añaden \emph{filas ficticias} si $N < \sum_k n_k$ (robots
    insuficientes) o \emph{columnas ficticias} si $N > \sum_k n_k$ (robots en
    exceso, al menos uno queda inactivo); y se minimiza la distancia euclídea
    total robot--slot mediante el algoritmo húngaro sobre la matriz cuadrada
    resultante. La reasignación se repite periódicamente cada $\Delta T_c = 1\,\text{s}$
    para capturar cambios de posición y de tarea activa.
    La función objetivo ---minimizar distancia de asignación--- no
    coincide necesariamente con maximizar $\Theta$, por lo que T4 es una cota de
    referencia de eficiencia de asignación, no un límite superior absoluto del
    throughput.

  \item \textbf{T5 -- Método propuesto}: tres dinámicas acopladas con función
    de payoff \eqref{eq:payoff} y tasa de revisión \eqref{eq:revision_rate}.
\end{itemize}
...
\begin{table}[htbp]
\centering
\caption[Hipótesis, escenarios y métricas]{Correspondencia entre hipótesis, escenarios y métrica principal de contraste.}
\label{tab:hipescenario}
\begin{tabularx}{\textwidth}{>{\bfseries}p{0.18\textwidth}p{0.25\textwidth}Y}
\toprule
Hipótesis & Escenarios de contraste & Métrica principal \\
\midrule
H1 (convergencia Nash) & A\,(config.\ balanceada, $n_k{=}2$) &
  ...
  primer instante en que se satisface la condición \\
H2 (tasa modulada) & A, B (ablación $\mu_i$ constante vs.\ modulada) &
  $\Delta_{\mathrm{chg}}$ (reducción ${\geq}40\%$) \\
H3 (factibilidad) & B, C, D ($R$ variable) &
  $F \geq 0{,}85$; curva $D(R)$ \\
H4 (brecha $\Delta J$) & B, C, E (perturbación) &
  $\Delta J < 0{,}25$ / $0{,}40$ \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{enumerate}
  \item La tasa de factibilidad del tratamiento~T5 es $F \geq 0{,}85$ en los
    Escenarios~B y~C.
  \item El tiempo de formación de coalición $T_{\text{coal}}$ del tratamiento~T5
