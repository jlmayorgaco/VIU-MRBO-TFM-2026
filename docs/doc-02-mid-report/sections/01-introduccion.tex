...
y se completan, la flota debe reasignarse continuamente, formando coaliciones que
satisfagan la cardinalidad mínima de cada tarea. Se trata de un problema de asignación
...
formalmente la convergencia a coaliciones de tamaño mínimo. El aprendizaje por refuerzo \citep{bezerra2025learning} produce políticas
...
restricciones de formación específicas. Sin embargo, en los trabajos revisados
dentro de esta línea suele suponerse que el tamaño del grupo que realiza cada
tarea es fijo y conocido de antemano. Ninguno aborda el caso en que la tarea misma impone un requisito mínimo
...
Una de las preguntas que la escalera comparativa del trabajo formaliza es qué
...
mediante consenso promedio y obtiene así $\hat{x}_{ik}$. Hasta aquí, Smith y DRD
...
Las contribuciones de este TFM se organizan en cuatro niveles. Primero, se formula
una función de payoff con componente sigmoide decreciente para representar requisitos
mínimos de cardinalidad (cotas inferiores, en contraste con las cotas superiores de
\citet{martinezpiazuelo2022automatica}). En el caso base con $\Psi\equiv1$, esta
función mantiene una estructura de juego de congestión ponderado; con $\Psi(d)$ activo,
el sistema implementado se interpreta como una perturbación espacial que acopla
asignación y movimiento. Segundo, se adaptan y comparan el replicador distribuido y
las dinámicas de Smith bajo la misma arquitectura de estimación por consenso dinámico
de media, lo que permite aislar el efecto de la política de revisión. Tercero, se
incorpora una tasa de revisión espacialmente modulada para reducir reasignaciones
espurias durante la aproximación a las cargas y en régimen estacionario. Cuarto, se
caracteriza empíricamente el coste de descentralización frente a una referencia
centralizada replanificada, así como la degradación del desempeño al reducir el rango
de comunicación. En conjunto, estas contribuciones definen una arquitectura donde la
estimación de ocupación, la revisión estratégica y la navegación reactiva se actualizan
dentro de un mismo bucle temporal, sin coordinador central de asignación.
...
no se actualizan continuamente ante cambios en el entorno. Los métodos de
...
robots según las necesidades de cada carga; en el corpus revisado no se identificó
una formulación de este problema con ese grado de generalidad.
...
La afirmación del trabajo es deliberadamente acotada: bajo comunicación local y
...
El propósito es responder esa pregunta. Afirmar la superioridad del control
distribuido sobre el centralizado en todo régimen operativo queda fuera de su alcance.
