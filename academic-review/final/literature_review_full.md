...
Generada UTC: 2026-09-13T22:53:01.449863+00:00
...
1C contiene **2309** candidatos; la cola de texto completo contiene
**1150** registros.
Stage 2 verificó **244** objetos de texto completo
por identidad; **123** quedaron limitados al
resumen, **762** solo tienen metadatos y
...
fuerza/wrench y AMR/AGV logístico. La exportación de Web of Science está parcialmente reconciliada: se incorporaron los archivos declarados, pero F01 y F02 solo contienen sus primeros 50 registros; por ello no se declara exhaustividad.
...
| Nivel | Registros |
|---|---:|
| Texto completo verificado | 244 |
| Solo resumen | 123 |
| Solo metadatos | 762 |
| Error de recuperación | 21 |

Roles preliminares: `BASELINE`=20, `CONTEXT`=796, `CORE`=155, `ENABLING`=155, `SURVEY`=24.
...
El corpus de la cola contiene 562 registros
con señal de asignación/coalición y 110
con texto completo verificado. Los términos de coding más observados en textos
verificados para coordinación/arquitectura son: `distributed` (228), `multi-agent` (157), `decentralized` (156), `consensus` (95), `auction` (57), `decentralised` (35), `centralized` (35), `market-based` (25).
...
Las etiquetas iniciales identifican 93
registros con señales físicas de transporte/contacto y 23
con texto completo verificado. En el texto completo aparecen: `object` (115), `load` (90), `payload` (42), `manipulation` (31), `pushing` (19), `caging` (12), `rigid body` (11), `grasp` (10).
...
Se observan 90 señales de planificación,
seguridad o contexto industrial y 24
textos completos verificados. Los términos de ejecución más frecuentes son:
`navigation` (163), `formation` (118), `path planning` (111), `trajectory` (90), `motion planning` (69), `fault` (37), `failure` (30), `synchronization` (30).
...
Las familias observadas en textos completos incluyen: `auction` (115), `reinforcement learning` (69), `consensus-based` (55), `formation control` (44), `genetic algorithm` (40), `fuzzy` (35), `integer programming` (28), `milp` (27).
Los términos experimentales incluyen: `simulation` (180), `experimental` (136), `simulated` (96), `experiment` (81), `hardware` (50), `benchmark` (20), `warehouse` (19), `ros` (16).
...
8. A Distributed Task Allocation Algorithm for a Multi-Robot System in Healthcare Facilities (2015; [DOI](https://doi.org/10.1007/s10846-014-0154-2)) — rol `CORE`, evidencia `fulltext_verified`.
...
14. A Sequential Task Addition Distributed Assignment Algorithm for Multi-Robot Systems (2021; [DOI](https://doi.org/10.1007/s10846-021-01394-2)) — rol `CORE`, evidencia `fulltext_verified`.
15. A Stochastic Clustering Auction (SCA) for Centralized and Distributed Task Allocation in Multi-agent Teams (2009; [DOI](https://doi.org/10.1007/978-3-642-00644-9_31)) — rol `CORE`, evidencia `fulltext_verified`.
16. A Two-Level Clustered Consensus-Based Bundle Algorithm for Dynamic Heterogeneous Multi-UAV Multi-Task Allocation (2025; [DOI](https://doi.org/10.3390/s25216738)) — rol `ENABLING`, evidencia `fulltext_verified`.
17. A decentralized multi-agent unmanned aerial system to search, pick up, and relocate objects (2017; [DOI](https://doi.org/10.1109/ssrr.2017.8088150)) — rol `ENABLING`, evidencia `fulltext_verified`.
18. A distributed PI-based dynamic task allocation method for multi-AUV systems (2026; [DOI](https://doi.org/10.1007/s00500-025-10894-4)) — rol `CORE`, evidencia `fulltext_verified`.
19. A distributed approach for road clearance with multi-robot in urban search and rescue environment (2019; [DOI](https://doi.org/10.1007/s41315-019-00111-5)) — rol `CORE`, evidencia `fulltext_verified`.
20. A kinematically compatible framework for cooperative payload transport by nonholonomic mobile manipulators (2006; [DOI](https://doi.org/10.1007/s10514-005-9717-9)) — rol `CORE`, evidencia `fulltext_verified`.
21. A multi-robot collaborative manipulation framework for dynamic and obstacle-dense environments: integration of deep learning for real-time task execution (2025; [DOI](https://doi.org/10.3389/frobt.2025.1585544)) — rol `CORE`, evidencia `fulltext_verified`.
22. A novel approach to task assignment in a cooperative multi-agent design system (2015; [DOI](https://doi.org/10.1007/s10489-014-0640-z)) — rol `ENABLING`, evidencia `fulltext_verified`.
23. A novel marine predator algorithm for robot task allocation problem (2026; [DOI](https://doi.org/10.1017/s0263574726103531)) — rol `CORE`, evidencia `fulltext_verified`.
24. A semantically-informed multirobot system for exploration of relevant areas in search and rescue settings (2015; [DOI](https://doi.org/10.1007/s10514-015-9480-x)) — rol `ENABLING`, evidencia `fulltext_verified`.
